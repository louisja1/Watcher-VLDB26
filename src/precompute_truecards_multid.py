...
def precompute_trainingset(
    trainingset_filename, n_tabs, sel_cols, pgtypes, counting_sqls
):
    assert n_tabs in [2, 3, 4]

    print("Precomputing the true cards for Trainingset")
    assert "_wo_true_cards" in trainingset_filename
    new_trainingset_filename = trainingset_filename.replace("_trainingset_wo_true_cards", "_old_trainingset")

    placeholders = []
    join_placeholder = "true_join_card"
    for i in range(1, n_tabs + 1):
        placeholders.append(f"true_card{i}")
        join_placeholder += str(i)
    placeholders.append(join_placeholder)
    ...
    with open(trainingset_filename, "r") as fin, open(new_trainingset_filename, "w") as fout:
        ...
        for i in tqdm(range(len(lines))):
            ...
            assert len(bounds) == n_tabs
            for j in range(len(row)):
                if row[j] in placeholders:
                    assert row[j] in counting_sqls
                    counting_sql = copy.deepcopy(counting_sqls[row[j]])
                    for jj in range(n_tabs):
                        if str(jj + 1) in row[j]:
                            counting_sql = counting_sql\
                                .replace("{lb" + str(jj + 1) + "}", str(bounds[jj][0]))\
                                    .replace("{ub" + str(jj + 1) + "}", str(bounds[jj][1]))
                    cursor.execute(counting_sql)
                    row[j] = str(cursor.fetchall()[0][0])
            fout.write(",".join(row) + "\n")
    ...
    with open(log_filename, "r") as fin, open(new_log_filename, "w") as fout, open(log_cost_filename, "w") as flatency:
        for row_num, line in enumerate(tqdm(fin)):
            ...
            if op_type in ["I", "D"]:
                ...
                assert len(all_latency) == n_tabs + 1
                line = ",".join(row) + "\n"
...
if __name__ == "__main__":
    ...
    with open(json_filename, "r") as json_fin:
        ...
        old_log_filename = config["old_log_filename"]
        old_trainingset_filename = config["old_trainingset_filename"]
        ...
        precompute_trainingset(
            trainingset_filename=old_trainingset_filename,
            n_tabs=len(tabs),
            sel_cols=sel_cols,
            pgtypes=pgtypes,
